...
Step 1
Using the TemplateHTMLRenderer, you can pass the data to an HTML file,
and then display that data using Django’s native templating language called DTL, or Django Templating Language. 

To test this TemplateHTMLRenderer the menu items need to be displayed in an HTML file instead of JSON.
To use this renderer, you first import it from the rest_framework.renderers module in the views.py file.
You also need to import the renderer_classes decorator. 
...
Step 4
The final step is to map this function to an API endpoint in the urls.py file
so that it can be browsed as http://127.0.0.1:8000/api/menu.
...
Menu-items endpoint displays menu items in formatted HTML table
StaticHTMLRenderer
...
Welcome to the Little Lemon API Project greeting
CSV renderer
...
Step 1
DRF doesn’t come with a CSV renderer class by default.
So the first step is to install a popular third-party package using pipenv. 

>> ' pipenv install djangorestframework-csv '


Step 2 >> Import this renderer in the views.py file.

' from rest_framework_csv.renderers import CSVRenderer '

Step 3 >> Add the renderer using the renderer_classes decorator to convert an API endpoint to display CSV instead of JSON.
Add the following line of code in the menu-items function after the @api_view() decorator:
...
API endpoint output in Insomnia


#_________________________________
' 	YAML renderer '
Step 1
To display the output of your APIs in YAML, another popular data format,
you need to install the djangorestframework-yaml using pipenv.

>> ' pipenv install djangorestframework-yaml '

Step 2 >> To test it with the menu-items function, import this YAML renderer in the views.py file.

>> ' from rest_framework_yaml.renderers import YAMLRenderer '

Step 3 >> Pass the YAMLRenderer class inside the renderer_classes decorator, just below the api_view decorator above the menu-items function. 
...
the items will show in yaml format
API endpoint output in Insomnia that displays the menu items
